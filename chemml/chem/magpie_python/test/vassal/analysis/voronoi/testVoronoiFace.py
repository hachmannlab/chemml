...
class testVoronoiFace(unittest.TestCase):
    ...
    def test_initialize(self):
        ...
        self.assertAlmostEqual(0.5, face.get_face_distance(), delta=1e-6)
    ...
    def test_simple_assemble(self):
        ...
        # Check out properties.
        self.assertEqual(4, face_to_assemble.n_edges())
        self.assertAlmostEqual(1.0, face_to_assemble.get_area(), delta=1e-6)
    ...
    def test_intersections(self):
        ...
        self.assertAlmostEqual(1.5, test_face.get_area(), delta=1e-6)

        # Ensure that a non-intersecting face returns "None".
        self.assertEqual(None, test_face.compute_intersection(VoronoiFace(
        ...
        new_face = VoronoiFace(self.cell.get_atom(0), AtomImage(
            self.cell.get_atom(0), [1, 0, 0]), radical=True)
        self.assertAlmostEqual(1.0, test_face.get_cut_length(new_face),
                                delta=1e-6)

        new_edge = test_face.compute_intersection(new_face)
        self.assertTrue(new_edge is not None)
        self.assertEqual(new_face, new_edge.get_edge_face())
        self.assertEqual(test_face, new_edge.get_intersecting_face())
        self.assertTrue(test_face.is_closed())
        self.assertAlmostEqual(1.0, test_face.get_area(), delta=1e-6)

        # Compute intersection with a face that adds a new edge.
        test_face.assemble_face_from_faces(original_faces)
        new_face = VoronoiFace(self.cell.get_atom(0), AtomImage(
            self.cell.get_atom(0), [1, 1, 0]), radical=True)
        self.assertAlmostEqual(math.sqrt(2)/2, test_face.get_cut_length(
            new_face), delta=1e-6)
        ...
        self.assertEqual(new_face, new_edge.get_edge_face())
        self.assertEqual(test_face, new_edge.get_intersecting_face())
        self.assertEqual(5, test_face.n_edges())