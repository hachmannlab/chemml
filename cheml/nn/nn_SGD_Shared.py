...
class nn_psgd(object):
    """ (nn_psgd) 
    The implementation of neural networks with stochstic gradient descent and HogWild 
    algorithm for shared memory multiprocessing.
    ...
    def __init__(self, nneurons=[7], input_act_funcs='tanh',
                 learn_rate=0.001,rms_decay=0.9,n_epochs=10000,batch_size=256,
                 n_cores=1,n_hist=20,n_check=50,threshold=0.1, print_level=1):
    ...
    def back_prop_init(self,c_X_train,c_Y_train,c_X_test,c_Y_test,shared_weights,shared_hist,input_act_funcs):
        """
    ...
    def back_propagate(self,para_input):
        """
    ...
    def train(self,X_train,X_test,Y_train,Y_test):
        ...
        cweights, chist = initialize_weights([n_features] + self.nneurons +
            [n_outputs],self.n_hist*self.n_cores)

        big_batch_size = (N / self.n_cores) + 1
        para_inputs = [[i, j, j+big_batch_size,self.learn_rate,self.rms_decay,self.n_epochs,self.batch_size,self.n_cores,self.n_hist,
            self.n_check,self.threshold, self.print_level] for i,j in enumerate(range(0,N,big_batch_size))]
        pool = Pool(processes=self.n_cores, initializer=self.back_prop_init, initargs=(c_X_train,c_Y_train,c_X_test,c_Y_test,
            cweights, chist, self.input_act_funcs))
        pool.map(self.back_propagate, para_inputs)
...
if __name__ == '__main__':
    ...
    Y_train = Y[:N/2]
    Y_test = Y[N/2:]
    nn = nn_psgd([20,10],'tanh',learn_rate=0.001,n_epochs=50000,n_cores=2)
    nnet = nn.train(X_train,X_test,Y_train,Y_test)
    print np.mean(np.abs(nn.output(X_test,nnet) - Y_test),axis=0)
    plt.plot(X_test[:,0],nn.output(X_test,nnet)[:,0],'x')
