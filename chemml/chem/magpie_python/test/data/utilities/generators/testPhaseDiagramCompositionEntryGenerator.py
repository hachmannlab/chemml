import unittest
from chemml.chem.magpie_python.data.utilities.generators.PhaseDiagramCompositionEntryGenerator \
...
class testPhaseDiagramCompositionEntryGenerator(unittest.TestCase):
    ...
    def test_generate_alloy_compositions(self):
        ...
        self.assertEqual(1, len(comps[1]))
        self.assertEqual(3, len(comps[2]))
        self.assertEqual(3, len(comps[3]))
    ...
    def test_generate_crystal_compositions(self):
        ...
        self.assertEqual(1, len(comps[1]))
        self.assertEqual(5, len(comps[2]))
        self.assertEqual(4, len(comps[3]))

        # Try making entries.
        self.pg.set_elements_by_index([0, 1, 2])
        entries = self.pg.generate_entries()
        self.assertEqual(3 * 1 + 3 * 5 + 4, len(entries))

        self.pg.set_elements_by_index([0, 1, 2, 3])
        entries = self.pg.generate_entries()
        self.assertEqual(4 * 1 + 6 * 5 + 4 * 4, len(entries))
